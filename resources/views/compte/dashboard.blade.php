...
<div class="historique-transaction">
    ...
    <tr>
        ...
        <td><a href="{{ route('transaction.edit',['id'=>$compte->id, 'id_transaction' => $transaction->id]) }}">Modifier</a></td>
        <td>
            <form action="{{ route('transaction.destroy',['id'=>$compte->id, 'id_transaction' => $transaction->id]) }}" method="post">
                {{ csrf_field() }}
                {{ method_field('DELETE') }}
